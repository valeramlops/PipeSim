            ...
            "history": [
                {
                    ...
                    "detections_data": r.detections_data,
                    "result_url": (
...
class FastVideoUpload(BaseModel):
    filename: str
    video_id: str

@router.post("/upload_video_fast")
async def upload_video_fast(data: FastVideoUpload):
    """
    Instant endpoint. File is already on the disk
    """
    logger.info(f"Getted request on video processing: {data.filename}")

    # Forming path, where has the frontend already put the file
    unique_filename = f"{data.video_id}_{data.filename}"
    video_path = UPLOAD_VIDEO_DIR / unique_filename
    ...
    logger.info(f"Sending task in Celery (Redis)...")
    try:
        # Immediately assign the task to the worker
        task = process_video_task.delay(str(video_path), str(output_path))
        logger.info(f"Successfully! Celery accepted the task: {task.id}")
    except Exception as e:
        logger.error(f"Error: CELERY/REDIS: {e}")
        raise HTTPException(
            status_code=500,
            detail="Redis connection failed"
        )
    
    logger.info("Saving status into DB (Postgres)...")
    # Writing in DB
    try:
        async with async_session() as db:
            new_video = VideoRecord(
            ...
            db.add(new_video)
            await db.commit()
        logger.info("Successfully written in DB!")
    except Exception as e:
    ...
    return {
        "status": "processing",
        "task_id": task.id,
        "message": "Instant dispatch to worker"
    }